...
gana.prototype.get = function(flex) {
    ...
    var that = this;
    // log('GANA', that.lan);
    // if (flex.la == 'lan' && !that.lan) return; // почему это я написал?
    ...
    // filterSeven.call(that, flex); // FIXME: gacCati дает g.7, убрать
    // filterTwo.call(that, flex);  // вообще чепуха
...
function filterOne(flex) { // W.734
    ...
    _.each(terms, function(term) {
        if (term.length > flex.flex.length) return; // -ti, -nti - lat_par // FIXME: TODO: нет, так просто отбросить длинный term нельзя - -Ami, -ati
        if (u.endWith(flex.query, term)) first.call(that, flex, term);
    });
}

function log() { console.log.apply(console, arguments) } // ामी'
...
function filterTwo(flex) {
    // FIXME: TODO: так тут точно нельзя вычислять dhatu - в длинном составном здесь - начало и плюс конец. А где середина? И проверить во всех ост., кроме первой ганы.
    // u.p(flex);
    var that = this;
    var stem = flex.stem;
    ...
    if (stem == stem_I) {
        ...
        dhatus = sandhi.int(query, flex.flex, flex.cflex);
    } else {
        dhatus = [stem_I];
    }
    var agunas = [stem];
...
function seven(flex, term) {
    ...
    var stems = sandhi.int(cflex.query, cflex.flex, cflex.cflex);
