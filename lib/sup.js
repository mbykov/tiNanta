...
tinsup.prototype.nama = function(query) {
            ...
            for (var v in vars) {
                ...
                for (var key in keys) {
                    ...
                    terms.forEach(function(term, idx) {
                        ...
                        // if (gend == 'masc' && num == 'pl' && v == 'in') log('HASH', hash);
                    });
                } // keys
            }
        }
    ...
    for (var v in hash) {
        ...
        ms.forEach(function(m) {
            ...
            // смысл: 'an' только для слов на согласную, rAjan, nAman
            if (v == 'an' && !u.isConsonant(fin)) return;
            stem = [stem, m.dict].join('');
            var morph = {gend: m.gend, num: m.num, key: m.key};
            var stemkey = [stem, v].join('-');
            if (!res[stemkey]) {
