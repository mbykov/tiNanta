...
// var sandhi = require('../utils/sandhi');
...
function redupCons(flex) {
    ...
    //log(stem, 'a:', aug, 'h:', head, 't:', tail);
    // log('CONS', cons, 'first', first, 'sec', second, 'weak', weak, 'guna', guna, 'HEAD', head, 'TAIL', tail);

    if (stem[1] == 'े') { // e-case - to if-else?
        dhatu = [first, second, Const.virama].join('');
        dhatus.push(dhatu);
        // log('e-case', cons, 'first', first, 'sec', second, 'weak', weak, 'guna', guna, 'head', head, 'tail', tail, 'dhatu', dhatu);
    }
    ...
    if (/र/.test(head)) {
        ...
        dhatus.push(dhatu);
        //log('ra-head', cons, 'first', first, 'sec', second, 'weak', weak, 'guna', guna, 'head', head, 'tail', tail, 'dhatu', dhatu, 'keys', flex.keys);
        ...
        dhatu = u.addVirama(dhatu);
        dhatus.push(dhatu);
        // log('kf', cons, 'first', first, 'sec', second, 'weak', weak, 'guna', guna, 'head', head, 'tail', tail, 'dhatu', dhatu, 'keys', flex.keys);
        ...
        dhatus.push(dhatu);
        // log('U-va', cons, 'first', first, 'sec', second, 'weak', weak, 'guna', guna, 'head', head, 'tail', tail, 'dhatus', dhatus);
        ...
        // log('long-short', cons, 'first', first, 'sec', second, 'weak', weak, 'guna', guna, 'head', head, 'tail', tail, 'dhatus', dhatus);
    } else if (guna == weak) {
        _.each(weaks, function(vow) { // guna - jIv
            dhatu = [head, vow, tail].join('');
            dhatu = u.addVirama(dhatu);
        ...
        // log('w=st', cons, 'first', first, 'sec', second, 'weak', weak, 'guna', guna, 'head', head, 'tail', tail, 'dhatu', dhatu, 'keys', flex.keys);
    } else if (guna && u.isIN(u.aguna(guna), weak)) {
        // FIXME: weakLong
        dhatu = [head, weak, tail].join('');
        dhatu = u.addVirama(dhatu);
        dhatus.push(dhatu);
        // log('aguna', cons, 'first', first, 'sec', second, 'weak', weak, 'guna', guna, 'head', head, 'tail', tail, 'dhatu', dhatu);
        if (weak == 'ि') {
        ...
        dhatus.push(dhatu);
        // log('u-va', cons, 'first', first, 'sec', second, 'weak', weak, 'guna', guna, 'head', head, 'tail', tail, 'dhatu', dhatu);
        dhatus.push(dhatu);
        ...
        dhatus.push(dhatu);
        //log('no guna', cons, 'first', first, 'sec', second, 'weak', weak, 'guna', guna, 'head', head, 'tail', tail, 'keys', flex.keys, 'dhatus', dhatus);
        ...
        //log('w=st but i', cons, 'first', first, 'sec', second, 'weak', weak, 'guna', guna, 'head', head, 'tail', tail, 'dhatu', dhatu, 'keys', flex.keys);
    } else {
        //log('ELSE', flex.keys);
    }
    ...
    flex.dhatus = dhatus;
    // log('--redup cons dhatus--', stem, dhatus);
...
function redupVow(flex) {
    ...
    var guna = vows[1];
    var idx = _.indexOf(stem, second, 1); // 1 is for first == second
    //log('CONS', cons, 'first', first, 'sec', second, 'weak', weak, 'guna', guna, 'keys', flex.keys, 'HEAD', head, 'TAIL', tail);
    //log('VOW', vow, 'cons', cons, 'first', first, 'sec', second, 'weak', weak, 'guna', guna, 'keys', flex.keys);
    var head;
    var tail;
    ...
    //log('vow-dhatus', cons, 'first', first, 'sec', second, 'weak', weak, 'guna', guna, 'head', head, 'tail', tail, 'keys', flex.keys, 'dhatus', dhatus);
    flex.dhatus = dhatus;
